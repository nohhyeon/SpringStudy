...
@ComponentScan(basePackages = { "su.spring.dept" })
...
// HikariConfig 클래스의 객체를 생성한다.
		HikariConfig hikariConfig = new HikariConfig(); // 데이터베이스 드라이버 클래스를 설정한다.
		hikariConfig.setDriverClassName("net.sf.log4jdbc.sql.jdbcapi.DriverSpy"); // 데이터베이스 접속 URL을 설정한다.
		hikariConfig.setJdbcUrl("jdbc:log4jdbc:oracle:thin:@127.0.0.1:1521:xe"); // 데이터베이스 접속을 위한 사용자 이름을 설정한다.
...
//마이바티스의 SqlSessionFactoryBean을 생성하고 이 팩토리 빈은 마이바티스와 데이터베이스 간의 세션을 관리한다.
	@Bean
...
//트랜잭션 관리에 사용할 데이터 소스를 설정한다.
		dataSourceTransactionManager.setDataSource(dataSource); // 구성된 트랜잭션 관리자를 반환한다.
